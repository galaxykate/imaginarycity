...
var tracery = function() {
	...
	TraceryNode.prototype.expand = function(preventRecursion) {
		...
		if (!this.isExpanded) {
			...
			case 1:
				...
				this.parsedModifiers = this.modifiers.map(function(modRaw) {
					...
					if (modKey.indexOf("(") > 0) {
						...
						if (!results || results.length < 2) {
							...
							modKey = this.modifiers[i].substring(0, modName.indexOf("("));
						}
					}

					var modFxn = node.grammar.modifiers[modKey];
	...
	// Clear and reroll this node
	TraceryNode.prototype.reroll = function() {
		this.children = [];
		this.isExpanded = false;
	...
	TraceryNode.prototype.rebuild = function() {
		this.buildFinishedText();
		if (this.parent)
			this.parent.rebuild();
	};

	TraceryNode.prototype.buildFinishedText = function() {
		switch (this.type) {
		case 0:
			this.finishedText = this.raw;
			break;
		case 1:
			this.finishedText = this.children.map(function(child) {
				return child.finishedText;
			}).join("");

			for (var i = 0; i < this.parsedModifiers.length; i++) {
				var mod = this.parsedModifiers[i];
				// Missing modifier?
				if (!mod.fxn) {
					this.errors.push("Missing modifier " + mod.key);
					this.finishedText += "((." + mod.key + "))";
				} else {
					this.finishedText = mod.fxn(this.finishedText, mod.params);
				}
			}
	...
	TraceryNode.prototype.getAllNodes = function() {
		var nodes = [this];
		if (this.children) {
			for (var i = 0; i < this.children.length; i++) {
				nodes = nodes.concat(this.children[i].getAllNodes());
			}
		}
		return nodes;
	};
	...
	RuleSet.prototype.addRules = function(raw) {
		if (Array.isArray(raw)) {
			this.defaultRules = this.defaultRules.concat(raw);
		} else if ( typeof raw === 'string' || raw instanceof String) {
			this.defaultRules.push(raw);
		} else if (raw === 'object') {
			// TODO: support for conditional and hierarchical rule sets
		}
	};

	RuleSet.prototype.clearCurrentRules = function() {
		this.defaultRules = [];
	};
	...
	Symbol.prototype.addRules = function(rawRules) {
		this.stack[this.stack.length - 1].addRules(rawRules);
	};

	Symbol.prototype.clearCurrentRules = function() {
		this.stack[this.stack.length - 1].clearCurrentRules();
	};
	...
	Grammar.prototype.clearCurrentRules = function(key) {
		this.symbols[key].clearCurrentRules();
	};

	Grammar.prototype.addRules = function(key, rawRules) {
		...
		if (this.symbols[key] === undefined) {
			...
			this.symbols[key].addRules(rawRules);
		}
	};
